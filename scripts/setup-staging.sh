...
# Luvora Staging Setup Script
# Run this on your VPS to set up staging environment

echo "🚀 Luvora Staging Setup"
echo "========================"
echo ""
...
# Application URLs
# ===========================================
NEXT_PUBLIC_APP_URL=https://staging.luvora.love
NEXT_PUBLIC_POCKETBASE_URL=https://staging-api.luvora.love
...
if pm2 describe luvora-staging-3003 > /dev/null 2>&1; then
  echo "PM2 process exists, reloading..."
  pm2 reload luvora-staging-3003
else
  echo "Starting PM2 process..."
  pm2 start ecosystem.config.js --only luvora-staging-3003
fi
pm2 save
...
if curl -f http://localhost:3003/api/health > /dev/null 2>&1; then
  ...
  echo "   Check logs: pm2 logs luvora-staging-3003"
fi
...
echo "   pm2 restart luvora-staging-3003"
echo ""
echo "3. Set up Nginx reverse proxy (see docs/DEPLOYMENT.md)"
echo "4. Get SSL certificate: sudo certbot --nginx -d staging.luvora.love"
echo ""
echo "5. View logs:"
echo "   pm2 logs luvora-staging-3003"
echo ""
