...
// Constants for timeouts (module-level to avoid re-creation)
...
function AutomationTabContent({
  ...
  const fetchChannels = async (signal?: { cancelled: boolean }) => {
                    ...
                    <div className="p-5 flex flex-col items-center text-center space-y-3">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${
                        ...
                        <svg className={`w-8 h-8 transition-colors ${
                          selectedPlatform === 'telegram'
                            ? 'text-primary'
                            : 'text-sky-400 group-hover:text-primary'
                        }`} viewBox="0 0 24 24" fill="currentColor">
...
function DashboardContent() {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'preferences' | 'automation' | 'history'>(
    (initialTab === 'automation' || initialTab === 'profile' || initialTab === 'preferences' || initialTab === 'history') ? initialTab : 'profile'
  );
